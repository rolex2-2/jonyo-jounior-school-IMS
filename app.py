...
def parent_dashboard():
    """Render the parent dashboard with linked students and announcements."""
    if current_user.role != 'parent':
        logger.warning(f"Unauthorized access to parent dashboard by user {current_user.id} with role {current_user.role}")
    ...
    try:
        # Fetch parent name
        parent_name = current_user.username  # Adjust if User model has a name field
        ...
        db_session.rollback()
        logger.error(f"Database error in parent_dashboard: {str(e)}")
        ...
        return render_template(
            ...
            recent_announcements=[],
            term_info={},
            content_data={'mission': '', 'vision': '', 'about': ''}
        )
    except Exception as e:
        db_session.rollback()
        logger.error(f"Unexpected error in parent_dashboard: {str(e)}")
...
@app.route('/link_student', methods=['GET', 'POST'])
@login_required
def link_student():
...
def view_users():
    ...
    # Initialize default values for term_info and content_data
    term_info = {}
    content_data = {'mission': '', 'vision': '', 'about': ''}
    ...
    try:
        if request.method == 'POST':
            ...
            if user:
                ...
                try:
                    ...
                    db_session.rollback()
                    flash('Username or admission number already exists.', 'danger')
        ...
        users = db_session.query(
            User.id, User.username, User.role, User.grade, User.admission_no, User.password_hash
        ).order_by(User.role, User.grade).all()

        # Safely fetch term_info and content_data
        try:
            term_info, content_data = fetch_common_data()
        except Exception as e:
            logger.warning(f"Failed to fetch common data: {e}")
        ...
        for grade in ['learners_grade7', 'learners_grade8', 'learners_grade9']:
            grouped_users[grade] = sorted(
                grouped_users[grade], key=lambda u: int(u.admission_no) if u.admission_no else float('inf')
            )
...
def manage_performance_levels():
    ...
    try:
        ...
        if edit_id:
            ...
            existing_level = db_session.query(
                PerformanceLevels
            ).filter_by(
                id=edit_id
            ).first()
            ...
            if not existing_level:
                ...
                flash(
                    'Performance level not found!',
                    'danger'
                )

                return redirect(
                    url_for(
                        'manage_performance_levels'
                    )
                )

            # Fill the form only when opening edit page
            if request.method == 'GET':

                form.min_marks.data = existing_level.min_marks

                form.max_marks.data = existing_level.max_marks

                form.level.data = existing_level.level

                form.points.data = existing_level.points

                form.comment.data = existing_level.comment

                form.type.data = existing_level.type
        ...
        if form.validate_on_submit():
            ...
            if type_ not in valid_types:
                ...
                flash(
                    'Invalid performance level type!',
                    'danger'
                )
            ...
            overlap_query = db_session.query(
                ...
                PerformanceLevels.min_marks <= max_marks,

                PerformanceLevels.max_marks >= min_marks
            ...
            if overlap:
                ...
                flash(
                    ...
                    'This marks range overlaps with another performance level of the same type!',
            ...
            if edit_id:
                ...
                performance_level = db_session.query(
                ...
                performance_level.min_marks = min_marks

                performance_level.max_marks = max_marks

                performance_level.level = level

                performance_level.points = points

                performance_level.comment = comment

                performance_level.type = type_
                ...
                flash(
                    ...
                    f'Performance level "{level}" updated successfully!',
            ...
            # =====================================
                ...
                flash(
                    ...
                    f'Performance level "{level}" added successfully!',
        ...
        for level in performance_levels:
            ...
            if level_type in grouped_levels:

                grouped_levels[level_type].append(level)


        return render_template(

            'manage_performance_levels.html',
        ...
        logger.error(
            ...
            f'Error in manage_performance_levels: '
        ...
        # IMPORTANT:
        # Pass grouped_levels even when an error occurs
...
def student_download_report_card(admission_no=None, grade=None, term=None, year=None, exam_type=None):
    ...
    try:
        ...
        if not student:
            logger.warning(f"No student found for admission_no={admission_no}")
            flash('Student profile not found.', 'danger')
            redirect_url = 'student_dashboard' if current_user.role == 'student' else 'parent_dashboard'
        ...
        report_form = ReportCardForm(
            ...
            grade=student.grade,
            term=default_term,
            year=int(default_year) if str(default_year).isdigit() else datetime.now().year,
            exam_type=default_exam_type
        )

        # Set form choices
        report_form.admission_no.choices = [(student.admission_no, f"{student.name} ({student.admission_no})")]
        report_form.grade.choices = [(g[0], g[1]) for g in GRADES if g[0] != 'all']
        report_form.term.choices = TERMS
        report_form.exam_type.choices = EXAM_TYPES
        ...
        if marks:
            ...
            try:
                rank_query = db_session.query(
                    ...
                    Marks.year == str(year) if year else True,
...
# ============================================================
# PERFORMANCE LEVEL HELPERS (DB-DRIVEN) – FIXED
# ============================================================

def _get_matching_performance_level(marks, type_='learning_area'):
    """
    Internal helper: return the PerformanceLevels row that matches
    the given marks and type, or None.
    """
    try:
        ...
        db_session = next(get_db())
        try:
            level = db_session.query(PerformanceLevels).filter(
                PerformanceLevels.type == type_,
                PerformanceLevels.min_marks <= marks,
                PerformanceLevels.max_marks >= marks
            ).order_by(PerformanceLevels.min_marks.desc()).first()
            return level
        finally:
            db_session.close()
    except Exception as e:
...
def get_performance_levels(marks, type_='learning_area'):
    ...
    try:
        ...
        # 0-100 limit applies ONLY to individual learning-area scores
        if type_ == 'learning_area' and marks > 100:
            logger.warning(f"Invalid marks for performance level: {marks}, type={type_}")
            return 'N/A'

        level_row = _get_matching_performance_level(marks, type_)
        if level_row and level_row.level:
            return level_row.level.strip()
        return 'N/A'
    except Exception as e:
...
def get_points(marks, type_='learning_area'):
    ...
    try:
        ...
        level_row = _get_matching_performance_level(marks, type_)
        if level_row and level_row.points is not None:
            return float(level_row.points)
        return 0.0
    except Exception as e:
...
def get_teacher_comment(level):
    """
    Return teacher comment based on performance level name.
    Looks up the comment stored in PerformanceLevels for that level.
    """
    try:
        ...
        db_session = next(get_db())
        try:
            row = db_session.query(PerformanceLevels).filter(
                func.lower(PerformanceLevels.level) == level.lower().strip()
            ).first()
...
def get_class_teacher_comment(total_marks, grade):
    """Generate class teacher comment based on TOTAL marks (DB-driven)."""
    try:
...
def get_principal_comment(total_marks):
    """Generate principal comment based on TOTAL marks (DB-driven)."""
    try:
...
def get_rank(admission_no, grade, term, year, exam_type):
    ...
    try:
        ...
        try:
            ...
            rank_val = rank_list.index(admission_no) + 1 if admission_no in rank_list else 'N/A'
            logger.debug(f"Rank for {admission_no} in {grade}, {term}, {year}, {exam_type}: {rank_val}")
            return str(rank_val)
        finally:
            db_session.close()
...
def generate_individual_report_card(admission_no, term, year, exam_type):
    ...
    try:
        ...
        term = str(term).strip()
        year = str(year).strip()
        exam_type = str(exam_type).strip().lower()

        student = db_session.query(Student).filter(
            func.lower(Student.admission_no) == admission_no.lower()
        ).first()
        if not student:
            logger.warning(f"No student found for admission_no={admission_no}")
            return None
        ...
        student_tuple = (
            student.id,
            student.name.strip() if student.name else 'Unknown',
            None,
            None,
            student.grade.strip() if student.grade else 'Unknown',
            student.admission_no.strip()
        )
        grade = student.grade.strip() if student.grade else ''

        learning_areas = get_learning_areas(grade, db_session)

        exam_type_norm = exam_type.replace('cat 1', 'cat1').replace('cat 2', 'cat2').replace('cat 3', 'cat3') \
        ...
        marks = db_session.query(
            Marks.admission_no, Marks.learning_area, Marks.total_marks,
            Marks.term, Marks.year, Marks.grade, Marks.exam_type
        ).filter(
            func.lower(Marks.admission_no) == admission_no.lower(),
            func.lower(Marks.grade) == grade.lower() if grade else True,
            func.lower(Marks.term) == term.lower() if term else True,
            Marks.year == year,
            func.lower(Marks.exam_type) == exam_type_norm,
            Marks.learning_area.in_(learning_areas) if learning_areas else True,
            Marks.total_marks.isnot(None)
        ).all()
        ...
        for m in marks:
            try:
                val = float(m.total_marks) if m.total_marks is not None else None
                if val is not None and 0 <= val <= 100:
                    valid_marks.append((
                        m.admission_no.strip(),
                        m.learning_area.strip(),
                        val,
                        m.term.strip() if m.term else '',
                        m.year,
                        m.grade.strip() if m.grade else '',
                        m.exam_type.strip() if m.exam_type else ''
                    ))
            except (ValueError, TypeError):
                continue

        fees = db_session.query(
            Fee.admission_no, Fee.total_fee, Fee.amount_paid, Fee.balance,
            Fee.grade, Fee.term, Fee.year
        ).filter(
            func.lower(Fee.admission_no) == admission_no.lower(),
            func.lower(Fee.grade) == grade.lower() if grade else True,
            func.lower(Fee.term) == term.lower() if term else True,
            Fee.year == year
        ).all()
        ...
        for f in fees:
            try:
                ...
                valid_fees.append((
                    f.admission_no.strip(),
                    total_fee,
                    amount_paid,
                    balance,
                    f.grade.strip() if f.grade else '',
                    f.term.strip() if f.term else '',
                    f.year
                ))
            except (ValueError, TypeError):
                continue
        ...
        if not valid_fees:
            valid_fees = [(admission_no, 0.0, 0.0, 0.0, grade, term, year)]

        rank = get_rank(admission_no, grade, term, year, exam_type_norm)
        total_students = db_session.query(func.count(Student.id)).filter(
            func.lower(Student.grade) == grade.lower() if grade else True
        ).scalar() or 1
        ...
        logger.error(f"Error in generate_individual_report_card for {admission_no}: {str(e)}\n{traceback.format_exc()}")
        return None
    finally:
        db_session.close()
    ...
    try:
        ...
        for student in students:
            try:
                ...
                logger.debug(f"Marks for {admission_no}: {len(student_marks)} entries")
                ...
                if table_y < 100:
                    ...
                    rows_per_page = max(5, int((600 - 100) / 20))
                ...
                # ★ FIXED: Use TOTAL marks (not average) so the lookup matches
                # the class_teacher / principal ranges stored in PerformanceLevels
                performance_level = (
                    get_performance_levels(total_marks, 'class_teacher')
                    if valid_subjects > 0 else 'N/A'
                )
                class_teacher_comment = (
                    get_class_teacher_comment(total_marks, student_grade)
                    if valid_subjects > 0 else 'No performance data available'
                )
                principal_comment = (
                    get_principal_comment(total_marks)
                    if valid_subjects > 0 else 'No performance data available'
                )
        ...
        if form.validate_on_submit():
            ...
            for s in students:
                ...
                try:
                    student_tuple = (
                        s.id,
                        s.name.strip() if s.name else 'Unknown',
                        None,
                        None,
                        s.grade.strip() if s.grade else 'Unknown',
                        s.admission_no.strip()
                    )
            ...
            for m in marks:
                try:
                    marks_value = float(m.total_marks) if m.total_marks is not None else None
                    if marks_value is not None and 0 <= marks_value <= 100:
                        valid_marks.append((
                            m.admission_no.strip(),
                            m.learning_area.strip(),
                        ...
                        invalid_marks.append((m.admission_no, m.learning_area, m.total_marks))
                except (ValueError, TypeError):
                    invalid_marks.append((m.admission_no, m.learning_area, m.total_marks))
            if invalid_marks:
            ...
            rank_query = db_session.query(
                Marks.admission_no, func.sum(Marks.total_marks).label('total')
            ).filter(
                func.lower(Marks.grade) == grade.lower().strip(),
                func.lower(Marks.term).in_(term_variations),
                Marks.year == year,
                ...
                func.lower(Marks.learning_area).in_([la.lower().strip() for la in learning_areas]),
                Marks.admission_no != None
            ).group_by(Marks.admission_no).order_by(func.sum(Marks.total_marks).desc())
...
@app.route('/download_all_report_cards_pdf', methods=['GET', 'POST'])
@login_required
def download_all_report_cards_pdf():
    """Download all report cards for a grade as a single multi-page PDF (admin only)."""
    if current_user.role != 'admin':
        logger.warning(f"Unauthorized access attempt by user {current_user.id} with role {current_user.role}")
        flash("Only admins can download report cards.", 'danger')
        return render_template('download_report_card.html', form=ReportCardForm(), term_info=fetch_common_data()[0])
    ...
    try:
        ...
        if form.validate_on_submit():
            ...
            logger.debug(f"Form submitted (single PDF): role={current_user.role}, grade={grade}, term={term}, year={year}, exam_type={exam_type}")
            ...
            learning_areas = get_learning_areas(grade, db_session) if 'db_session' in dir(get_learning_areas) else get_learning_areas(grade)
            if not learning_areas:
                logger.warning(f"No learning areas for grade {grade}")
                flash(f"No learning areas defined for grade {grade}. Please contact the system admin.", 'danger')
                return render_template('download_report_card.html', form=form, term_info=term_info, content_data=content_data)
            ...
            if not valid_marks:
                flash(f"No valid marks data found for the selected criteria.", 'danger')
                return render_template('download_report_card.html', form=form, term_info=term_info, content_data=content_data)
            ...
            for i, r in enumerate(rank_query_results):
                try:
                    total_m = float(r.total) if r.total is not None else 0.0
                    if 0 <= total_m <= (100 * len(learning_areas)):
                        ranks[r.admission_no.strip().lower()] = i + 1
                except (ValueError, TypeError):
                    continue
            total_students = len(ranks) or len(students_formatted)

            # Generate the single multi-page PDF
            term_info_dict = {
                'term': term.capitalize(),
                'year': year,
                'principal': get_principal_name(),
            ...
            pdf_buffer = generate_report_card(
                ...
                total_students=total_students,
                grade=grade,
                term_info=term_info_dict
            )

            if not pdf_buffer or len(pdf_buffer.getvalue()) == 0:
                logger.error(f"Failed to generate single PDF report cards for grade {grade}")
                flash(f"Could not generate report cards PDF for grade {grade}. Check database data or contact system admin.", 'danger')
                return render_template('download_report_card.html', form=form, term_info=term_info, content_data=content_data)

            pdf_content = pdf_buffer.getvalue()
            if not pdf_content.startswith(b'%PDF-'):
                logger.error(f"Invalid PDF generated for grade {grade}")
                pdf_buffer.close()
                flash("Generated PDF is invalid. Please contact system admin.", 'danger')
                return render_template('download_report_card.html', form=form, term_info=term_info, content_data=content_data)

            filename = f'Report_Cards_{grade.replace(" ", "_")}_{term}_{year}_{exam_type}.pdf'
            logger.info(f"Admin {current_user.id} downloaded single-PDF report cards for grade {grade}, size={len(pdf_content)} bytes")
        ...
        db_session.rollback()
        logger.error(f"Database error in download_all_report_cards_pdf: {str(e)}\n{traceback.format_exc()}")
        flash('Database error while generating report cards. Please contact the system admin.', 'danger')
        return render_template('download_report_card.html', form=form, term_info=term_info, content_data=content_data)
    except Exception as e:
        db_session.rollback()
        logger.error(f"Unexpected error in download_all_report_cards_pdf: {str(e)}\n{traceback.format_exc()}")
        flash(f"Error generating report cards: {str(e)}. Please try again or contact system admin.", 'danger')
        return render_template('download_report_card.html', form=form, term_info=term_info, content_data=content_data)
    finally:
        db_session.close()


@app.route('/download_general_excel_results', methods=['GET', 'POST'])
@login_required
@admin_required
def download_general_excel_results():
    """
    Download general Excel results for a grade.
    Columns: Admission No, Name,
             then for every learning area → Marks + Performance Level,
             then Total Marks, Total Points, Average Points,
             Overall Performance Level, Rank.
    """
    form = ReportCardForm()
    term_info, content_data = fetch_common_data()
    db_session = next(get_db())
    ...
    try:
        ...
        if form.validate_on_submit():
            ...
            logger.debug(
                f"General Excel results requested: grade={grade}, term={term}, "
                f"year={year}, exam_type={exam_type}"
            )

            # ---------- Students ----------
            students_q = db_session.query(
                Student.id, Student.name, Student.grade, Student.admission_no
            )
            if grade.lower() != 'all':
                students_q = students_q.filter(func.lower(Student.grade) == grade.lower())
            students = students_q.order_by(Student.admission_no).all()
            ...
            # ---------- Learning areas ----------
            learning_areas = get_learning_areas(grade if grade.lower() != 'all' else 'Grade 7')
            if not learning_areas:
                flash(f"No learning areas defined for {grade}.", 'danger')
                return render_template(
                    'download_results.html',
                    form=form,
                    term_info=term_info,
            ...
            if grade.lower() != 'all':
                marks_q = marks_q.filter(func.lower(Marks.grade) == grade.lower())

            marks = marks_q.all()

            # ---------- Generate Excel ----------
            excel_buffer = generate_general_excel_results(
                students=students,
                marks=marks,
            ...
            filename = (
                f"General_Results_{grade.replace(' ', '_')}_"
                f"{term}_{year}_{exam_type}.xlsx"
            )
        ...
        # GET request
        form.term.data = term_info.get('term', 'Term 1')
        form.year.data = default_year
        return render_template(
            'download_results.html',
            form=form,
        ...
        logger.error(f"Error in download_general_excel_results: {str(e)}\n{traceback.format_exc()}")
        flash(f"Error: {str(e)}", 'danger')
        return render_template(
            'download_results.html',
            form=form,
...
def generate_general_excel_results(students, marks, learning_areas, grade,
                                   term=None, year=None, exam_type=None):
    """
    Generate Excel with structure:

    Admission No | Name |
    [LA1 Marks | LA1 Level] [LA2 Marks | LA2 Level] ... |
    Total Marks | Total Points | Average Points | Overall Perf. Level | Rank
    """
    try:
        wb = Workbook()
        ws = wb.active
        ws.title = f"Results_{grade.replace(' ', '_')[:25]}"
        ...
        if marks:
            ...
            for m in marks:
                adm = str(m[0]).strip().lower()
                try:
                    val = float(m[2]) if m[2] is not None else 0
                    if 0 <= val <= 100:
                        totals[adm] += val
                except (ValueError, TypeError):
                    continue

            sorted_adms = sorted(totals.items(), key=lambda x: x[1], reverse=True)
            for i, (adm, _) in enumerate(sorted_adms):
                rank_data[adm] = i + 1
        ...
        for student in students:
            try:
                # Support different student formats
                if isinstance(student, (list, tuple)) and len(student) >= 4:
                    # (id, name, grade, admission_no)  – common in routes
                    admission_no = str(student[3]).strip().lower()
                    name = str(student[1]).strip()
                    student_grade = str(student[2]).strip()
                elif isinstance(student, (list, tuple)) and len(student) >= 6:
                    # (id, name, None, None, grade, admission_no)
                    admission_no = str(student[5]).strip().lower()
                    name = str(student[1]).strip()
                ...
                if admission_no in processed:
                    continue
                processed.add(admission_no)

                # Collect marks for this student
                student_marks = {}
                for m in marks:
                    m_adm = str(m[0]).strip().lower()
                    if m_adm != admission_no:
                        continue
                    la = str(m[1]).strip()
                    try:
                        val = float(m[2]) if m[2] is not None else None
                        if val is not None and 0 <= val <= 100:
                            student_marks[la.lower()] = val
                    except (ValueError, TypeError):
                        continue

                # Build row
                row = [admission_no.upper(), name]
                total_marks = 0.0
                total_points = 0.0
                valid_subjects = 0

                for la in learning_areas:
                    marks_value = student_marks.get(la.lower().strip())
                    if marks_value is not None:
                        level = get_performance_levels(marks_value, 'learning_area')
                        points = get_points(marks_value, 'learning_area')
                        row.extend([int(marks_value), level])
                        total_marks += marks_value
                        total_points += points
                        valid_subjects += 1
                    else:
                ...
                average_points = total_points / valid_subjects if valid_subjects > 0 else 0.0
                overall_level = (
                    get_performance_levels(total_marks, 'class_teacher')
                    if total_marks > 0 else 'N/A'
                )
                rank = rank_data.get(admission_no, 'N/A')

                row.extend([
                    int(total_marks) if total_marks > 0 else '',
                    f"{total_points:.2f}" if total_points > 0 else '',
                    f"{average_points:.2f}" if valid_subjects > 0 else '',
                    overall_level,
                    rank
                ])
                ws.append(row)
                ...
                logger.error(f"Error processing student in general Excel: {str(e)}")
                continue

        if not processed:
            ws.append(['No students found for this selection.'])
        ...
        if len(buffer.getvalue()) == 0:
            logger.error("Generated empty general Excel file")
            return None

        logger.debug(f"General Excel generated for {grade}, size={len(buffer.getvalue())} bytes")
        return buffer

    except Exception as e:
        logger.error(f"Error in generate_general_excel_results: {str(e)}\n{traceback.format_exc()}")
        return None
