...
from database import (
    get_db, User, Student, Marks, Fee, TermInfo,
    TeacherAssignments, ClassTeachers, PerformanceLevels
)
import uuid
...
def generate_teacher_password():
    ...
    password = f"{letter}-{numbers}"
    logger.debug(f"Generated teacher password: {password}")
    return password
...
def generate_student_password(name, admission_no):
    ...
    try:
        first_name = name.strip().split()[0] if name and isinstance(name, str) and name.strip() else 'S'
        initial = first_name[0].upper() if first_name else 'S'
        password = f"{initial}-{admission_no}"
        logger.debug(f"Generated student password for {name}, {admission_no}: {password}")
        return password
    except Exception as e:
        logger.error(f"Error generating student password for {name}, {admission_no}: {str(e)}")
        return f"S-{admission_no}"
...
def generate_individual_report_card(admission_no, term, year, exam_type):
    ...
    try:
        ...
        if not year.isdigit() or not (2000 <= int(year) <= 2100):
            ...
            raise ValueError("Year must be a valid four-digit number between 2000 and 2100")
        ...
        try:
            ...
            student = db_session.query(Student.name, Student.grade).filter(
                func.lower(Student.admission_no) == admission_no
            ).first()
            if not student:
                logger.warning(f"No student found for admission_no={admission_no}")
                from flask import flash
                flash('Student not found. Please contact the administrator.', 'danger')
                return None
            name, grade = student
            logger.debug(f"Student: name={name}, grade={grade}")
            ...
            marks = db_session.query(
                ...
                func.lower(Marks.admission_no) == admission_no,
                func.lower(Marks.grade) == grade.lower(),
                func.lower(Marks.term) == term,
                Marks.year == year,
                func.lower(Marks.exam_type) == exam_type,
            ...
            logger.debug(f"Fetched {len(marks)} valid marks for {admission_no}: {[m[:3] for m in marks]}")

            # Check for valid marks
            if not marks:
                logger.warning(f"No valid marks found for {admission_no}, grade={grade}, term={term}, year={year}, exam_type={exam_type}")
                from flask import flash
                flash('No valid marks available for the selected criteria. Please contact your teacher.', 'warning')
                return None
            ...
            fees = db_session.query(
                ...
                func.lower(Fee.term) == term,
                Fee.year == year
            ).all()
            fees = [
                (f.admission_no, f.total_fee, f.amount_paid, f.balance, f.grade, f.term, f.year)
                for f in fees
                if f.total_fee is not None and f.amount_paid is not None and f.balance is not None
            ]
            logger.debug(f"Fetched {len(fees)} valid fees for {admission_no}: {[f[:4] for f in fees]}")
            ...
            if pdf_buffer is None or len(pdf_buffer.getvalue()) == 0:
                ...
                from flask import flash
                flash('Error generating report card. Please try again later.', 'danger')
                return None
...
def generate_excel_results(students, marks, grade, term=None, year=None, exam_type=None):
    ...
    Layout:
    Admission No | Name | [LA Marks | LA Level] × N | Total Marks | Total Points | Average Points | Performance Level | Rank
    """
    try:
        wb = Workbook()
        ws = wb.active
        ...
        learning_areas = sorted(set(
            m.learning_area if isinstance(m, Marks) else m[1] if isinstance(m, (list, tuple)) else m.get('learning_area', '')
            for m in marks
            if (isinstance(m, Marks) and m.grade.lower().strip() == grade.lower().strip()) or
               (isinstance(m, (list, tuple)) and len(m) >= 6 and str(m[5]).lower().strip() == grade.lower().strip()) or
               (isinstance(m, dict) and str(m.get('grade', '')).lower().strip() == grade.lower().strip())
        ))
        headers = ['Admission No', 'Name']
        for la in learning_areas:
            headers.extend([f"{la} Marks", f"{la} Level"])
        headers.extend(['Total Marks', 'Total Points', 'Average Points', 'Performance Level', 'Rank'])
        ws.append(headers)
        ...
        for student in students:
            try:
                if isinstance(student, (list, tuple)) and len(student) >= 6:
                    ...
                    admission_no = str(student.admission_no).strip().lower()
                    name = str(student.name).strip()
                    student_grade = str(student.grade).strip().lower()
                else:
                    logger.warning(f"Invalid student data: {student}")
                    continue

                if grade != 'all' and student_grade != grade.lower():
                    continue
                if admission_no in processed_students:
                    continue
                processed_students.add(admission_no)
                ...
                student_marks = {
                    str(m.learning_area if isinstance(m, Marks) else m[1] if isinstance(m, (list, tuple)) else m.get('learning_area', '')).strip().lower():
                    float(m.total_marks if isinstance(m, Marks) else m[2] if isinstance(m, (list, tuple)) else m.get('total_marks', 0))
                    for m in marks
                    if ((isinstance(m, Marks) and str(m.admission_no).strip().lower() == admission_no) or
                        (isinstance(m, (list, tuple)) and len(m) >= 3 and str(m[0]).strip().lower() == admission_no) or
                        (isinstance(m, dict) and str(m.get('admission_no', '')).strip().lower() == admission_no)) and
                       ((isinstance(m, Marks) and str(m.grade).lower().strip() == grade.lower()) or
                        (isinstance(m, (list, tuple)) and len(m) >= 6 and str(m[5]).lower().strip() == grade.lower()) or
                        (isinstance(m, dict) and str(m.get('grade', '')).lower().strip() == grade.lower())) and
                       m.total_marks is not None and 0 <= float(m.total_marks if isinstance(m, Marks) else m[2] if isinstance(m, (list, tuple)) else m.get('total_marks', 0)) <= 100
                }
                total_marks = sum(m for m in student_marks.values() if m is not None)
                total_points = sum(
                    get_points(float(m.total_marks if isinstance(m, Marks) else m[2] if isinstance(m, (list, tuple)) else m.get('total_marks', 0)), 'learning_area')
                    for m in marks
                    if ((isinstance(m, Marks) and str(m.admission_no).strip().lower() == admission_no) or
                        (isinstance(m, (list, tuple)) and len(m) >= 3 and str(m[0]).strip().lower() == admission_no) or
                        (isinstance(m, dict) and str(m.get('admission_no', '')).strip().lower() == admission_no)) and
                ...
                valid_subjects = len(student_marks)
                average_points = total_points / valid_subjects if valid_subjects > 0 else 0
                ...
                # Use total_marks for overall performance level (consistent with PDF)
                performance_level = get_performance_levels(total_marks, 'class_teacher') if total_marks > 0 else 'N/A'
                first_mark = next((m for m in marks if (isinstance(m, Marks) and str(m.admission_no).strip().lower() == admission_no) or
                                  (isinstance(m, (list, tuple)) and len(m) >= 3 and str(m[0]).strip().lower() == admission_no) or
                                  (isinstance(m, dict) and str(m.get('admission_no', '')).strip().lower() == admission_no)), None)
                rank_term = term if term else (first_mark.term if isinstance(first_mark, Marks) else first_mark[3] if first_mark else 'Term 1')
                rank_year = year if year else (first_mark.year if isinstance(first_mark, Marks) else first_mark[4] if first_mark else '2025')
                rank_exam_type = exam_type if exam_type else (first_mark.exam_type if isinstance(first_mark, Marks) else first_mark[6] if first_mark else 'endterm')
                rank = get_rank(admission_no, grade, rank_term, rank_year, rank_exam_type) if first_mark else 'N/A'
                row.extend([int(total_marks) if total_marks > 0 else '', f"{total_points:.2f}" if total_points > 0 else '', f"{average_points:.2f}" if valid_subjects > 0 else '', performance_level, rank])
                ws.append(row)
        ...
        try:
            for fee in fees:
                try:
                    ...
                    if grade and fee_grade != grade.lower():
                    ...
                    name = db_session.query(Student.name).filter(
                        func.lower(Student.admission_no) == admission_no
                    ).scalar() or 'Unknown'
                    ...
                    c.drawString(50, y, admission_no.upper())
...
def generate_fee_statement_excel(fees, grade=None, term=None, year=None, highlight_duplicates=False):
    ...
    try:
        ...
        try:
            for fee in fees:
                try:
                    ...
                    ws.append([admission_no.upper(), name, total_fee, amount_paid, balance])
        ...
        if highlight_duplicates:
            for row in ws.iter_rows(min_row=2, max_row=row_num-1):
                admission_no = row[0].value.lower()
